...
class Zip
{
	 ...
	 */
	public function search($zip, $isNew = true)
	{
		$zip = trim($zip);
	 ...
	 */
	private function _checkFormatNew(&$zip)
	{
		// 文字列長の取得
		$len = strlen($zip);
		...
		elseif ($len === 8) {
			// 区切り文字位置を除いた郵便番号にする
			$zip = substr($zip, 0, 3) . substr($zip, -4);
		}
		// それ以外の場合
		else {
			// フォーマットエラー
	 ...
	 */
	private function _checkFormatOld(&$zip)
	{
		// 半角数字3桁、または5桁以外
	 ...
	 *   [request_zip_num]=>	リクエストされた郵便番号
	 *   [request_zip_version]=>リクエストされた郵便番号形式(0 = 7桁, 1 = 3 or 5桁, なし = none)
	 ...
	 */
	private function _getCsvAll($csvPath, $toEnc = DESC_CHARSET, $fromEnc = SRC_CHARSET)
	{
		// 検索結果配列
